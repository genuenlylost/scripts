...
-- Fully functional with all UI buttons working
...
local function SafeTeleport(targetCFrame, callback)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if distance > 50 then
            local tweenInfo = TweenInfo.new(math.min(distance / 100, 3), Enum.EasingStyle.Linear, Enum.EasingDirection.InOut)
...
RunService.Heartbeat:Connect(function()
    ...
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
        ...
        if ScriptState.SpeedHack then
            humanoid.WalkSpeed = math.min(ScriptState.WalkSpeed, 35)
        else
            humanoid.WalkSpeed = 16
        end
        
        humanoid.JumpPower = math.min(ScriptState.JumpPower, 120)
    end
